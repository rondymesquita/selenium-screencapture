...
public class ImageDiff {
	...
	private void generateDiff() throws IOException {
		/*
		 * Generate the diff of two images If they are different, so the diff
		 * image is generated and saved
		 */
		BufferedImage diffLayer = generateDiffLayer();
		if (!areImagesEqual) {
	...
	private BufferedImage generateDiffLayer() {
		...
		diffLayer = new BufferedImage(maxWidth, maxHeight, BufferedImage.TYPE_INT_ARGB);
		Logger.logInfo("maxWidth: " + maxWidth);
		Logger.logInfo("maxHeight: " + maxHeight);
